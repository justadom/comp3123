...
express.Router() is basically a way to organize routes in your Express app. Instead of putting all your routes in index.js, you can create separate files for different parts of your app, like users.js for user-related routes.
...
In Express, you can handle errors with special middleware. If something goes wrong in a route (like a file not found), you can pass the error to this middleware using next(err).
#### Section C: Bonus
...
process.env.port is a variable set by the environment (like on a cloud server or hosting platform such as Heroku).

8081 is a fallback port if process.env.port isn’t set (like when you run it on your local computer).
